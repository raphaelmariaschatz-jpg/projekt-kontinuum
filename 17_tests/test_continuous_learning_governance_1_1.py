...
MODULE_PATH = ROOT / "12_agents" / "continuous_learning_governance_1_1.py"

spec = importlib.util.spec_from_file_location("continuous_learning_governance_1_1", MODULE_PATH)
module = importlib.util.module_from_spec(spec)
assert spec and spec.loader
sys.modules[spec.name] = module
spec.loader.exec_module(module)

ContinuousLearningGovernance11 = module.ContinuousLearningGovernance11
...
with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temporary:
    ...
    clg = ContinuousLearningGovernance11(root)
    assert clg.version == "1.1"
    ...
    assert "Continuous Learning Governance 1.1 Status Report" in report
    assert "Governance Status" in report
    assert "Queue Status" in report
    assert "Governance Environment" in report
    assert "Queue:" in report
    assert "33_learning" in report
    assert "learning_queue.json" in report
    assert "History:" in report
    assert "learning_history.json" in report
    assert "Audit:" in report
    assert "governance_events.json" in report
    assert "Version:" in report
    assert "CLG 1.1" in report
...
print("Kontinuum Continuous Learning Governance 1.1 tests passed")
