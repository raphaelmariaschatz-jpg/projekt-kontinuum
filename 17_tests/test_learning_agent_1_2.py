...
MODULE_PATH = ROOT / "12_agents" / "learning_agent_1_2.py"

spec = importlib.util.spec_from_file_location("learning_agent_1_2", MODULE_PATH)
module = importlib.util.module_from_spec(spec)
assert spec and spec.loader
sys.modules[spec.name] = module
spec.loader.exec_module(module)

LearningAgent12 = module.LearningAgent12
LearningSource = module.LearningSource


with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temporary:
    root = Path(temporary)
    agent = LearningAgent12(project_root=root)
    ...
    assert provenance["agent_version"] == "1.2"
    ...
    assert "Learning Agent 1.2 Status Report" in report
    assert "- Version: 1.2" in report
    assert "- Version: 1.1" not in report
    ...
    assert "## Kompatibilitaet" in report
    assert "Foundation Layer: kompatibel" in report
    assert "Governance Layer: kompatibel" in report
    assert "CAM: kompatibel" in report
    assert "Knowledge Agent: Handoff" in report
    assert "Memory Agent: Handoff" in report
    assert "## Proposal Lifecycle" in report
    assert "pending" in report
    assert "approved" in report
    assert "Knowledge Agent" in report
    assert "rejected" in report

print("Kontinuum Learning Agent 1.2 tests passed")
