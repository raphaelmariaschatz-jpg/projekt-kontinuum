...
with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temporary_root:
    ...
    try:
        status = system.ask("fundamentzyklenstatus")
        assert "Foundation-Zyklus-Diagnose" in status
        result = system.ask("fundamentzyklus reparieren")
        assert "Foundation-Zyklus-Recovery" in result
        assert system.status()["foundation_cycles"]["open"] == 0
    finally:
        system.close()

print(f"Kontinuum {APP_VERSION} foundation cycle recovery tests passed")
