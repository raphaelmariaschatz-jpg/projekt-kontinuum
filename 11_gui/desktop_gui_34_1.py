...
"""
Projekt Kontinuum 34.1 – Desktop-GUI
...
- Agentenstatus
- Systemstatus inklusive Backend-Version 34.1
- Release Integrity Framework mit externem, signiertem Freigabenachweis
- Autonomous-Diagnostics-Startmeldung und Schnellbefehl
...
class KontinuumGUI(tk.Tk):
    def __init__(self, identity: dict):
        ...
        self.file_drop_status = "nicht initialisiert"
    ...
    def _build_layout(self):
        ...
        output_frame.grid(row=0, column=0, sticky="nsew", pady=(0, 8))
        output_frame.rowconfigure(1, weight=1)
        output_frame.columnconfigure(0, weight=1)

        output_toolbar = ttk.Frame(output_frame)
        output_toolbar.grid(row=0, column=0, sticky="ew", pady=(0, 4))
        output_toolbar.columnconfigure(0, weight=1)
        ttk.Label(output_toolbar, text="Suchmodus:").grid(row=0, column=1, sticky="e", padx=(0, 6))
        self.search_mode = tk.StringVar(value="Automatisch")
        self.search_mode_dropdown = ttk.Combobox(
            output_toolbar,
            textvariable=self.search_mode,
            values=("Automatisch", "Lokal", "Internet", "Hybrid"),
            state="readonly",
            width=13,
        )
        self.search_mode_dropdown.grid(row=0, column=2, sticky="e", padx=(0, 8))
        ttk.Button(output_toolbar, text="Kopieren", command=self.copy_output).grid(row=0, column=3, sticky="e")

        self.output = ScrolledText(output_frame, wrap="none", font=("Segoe UI", 11))
        self.output.grid(row=1, column=0, sticky="nsew")
        output_xscroll = ttk.Scrollbar(output_frame, orient="horizontal", command=self.output.xview)
        output_xscroll.grid(row=2, column=0, sticky="ew")
        self.output.configure(xscrollcommand=output_xscroll.set)
        ...
        ttk.Button(button_row, text="Datei öffnen", command=self.open_file_prompt).grid(row=0, column=2, padx=(8, 0))
        ttk.Button(button_row, text="Datei lernen", command=self.learn_file_prompt).grid(row=0, column=3, padx=(8, 0))
        ttk.Button(button_row, text="Ordner lernen", command=self.learn_folder_prompt).grid(row=0, column=4, padx=(8, 0))
        ttk.Button(button_row, text="Eingabe löschen", command=self.clear_input).grid(row=0, column=5, padx=(8, 0))
        ...
        internet_learning = ttk.LabelFrame(side, text="Internet-Lernen", padding=8)
        internet_learning.grid(row=4, column=0, sticky="ew", pady=(0, 10))
        internet_learning.columnconfigure(1, weight=1)
        self.internet_learning_state = ttk.Label(internet_learning, text="Internet-Lernen: Aktiv")
        self.internet_learning_state.grid(row=0, column=0, columnspan=2, sticky="w")
        self.internet_learning_source = ttk.Label(internet_learning, text="Letzte Quelle: -")
        self.internet_learning_source.grid(row=1, column=0, columnspan=2, sticky="w")
        self.internet_learning_time = ttk.Label(internet_learning, text="Letzte Lernzeit: -")
        self.internet_learning_time.grid(row=2, column=0, columnspan=2, sticky="w")
        self.internet_learning_findings = ttk.Label(internet_learning, text="Neue Funde: 0")
        self.internet_learning_findings.grid(row=3, column=0, columnspan=2, sticky="w")
        self.internet_learning_limit = ttk.Label(internet_learning, text="Bandbreitenlimit aktiv: ja | Limit: 10 %")
        self.internet_learning_limit.grid(row=4, column=0, columnspan=2, sticky="w")
        self.internet_learning_button = ttk.Button(
            internet_learning,
            text="Internet-Lernen deaktivieren",
            command=self.toggle_internet_learning,
        )
        self.internet_learning_button.grid(row=5, column=0, columnspan=2, sticky="ew", pady=(6, 0))
        ...
        quick_commands = (
            ("Status", "status"),
            ("Agentenstatus", "agenten"),
            ("Internetstatus", "internetstatus"),
            ("Lernstatus", "lernstatus"),
            ("Modellstatus", "modellstatus"),
            ("Pythonstatus", "pythonstatus"),
            ("Wingetstatus", "wingetstatus"),
            ("Formelstatus", "formelstatus"),
            ("Notizbuchstatus", "notizbuchstatus"),
            ("Bedeutungsstatus", "bedeutungsstatus"),
            ("Bedeutungspfad", "bedeutungspfad identität"),
            ("Fundamentschicht", "fundamentschichtstatus"),
            ("Motivationsstatus", "motivationsstatus"),
            ("Motivationsprioritäten", "motivationsprioritäten"),
            ("Motivationserklärung", "motivationserklärung identität"),
            ("Wichtige Einflüsse", "wichtige einflüsse identität"),
            ("Relevanzstatus", "relevanzstatus"),
            ("Bedeutungsinflation", "bedeutungsinflation"),
            ("Sessionstatus", "sessionstatus"),
            ("Fundamentzyklen", "fundamentzyklenstatus"),
            ("Diagnostikstatus", "diagnostikstatus"),
            ("CCE Status", "canonicalenginestatus"),
        )
        for index, (label, command) in enumerate(quick_commands):
            ...
            ttk.Button(
                controls,
                text=label,
                command=lambda command=command: self.quick(command),
    ...
    def _write_system_message(self):
        ...
        self.log_activity("FileAgent", "Dateien per Button oder Drag-and-Drop ins Eingabefeld als Lernquelle übergeben")
        self.log_activity("FileAgent", self.file_drop_status)
        report = getattr(self.system, "diagnostic_report", None)
        if report:
            self.write_output("Kontinuum", report["message"])
            self.log_activity("Diagnostik", f"{report['count']} Befunde | Wichtigkeit {report['highest_severity']}")
        self.refresh_agents()
    ...
    def _refresh_status(self):
        try:
            ...
            self._refresh_internet_learning_status(st)
    ...
    def _refresh_internet_learning_status(self, status: dict):
        info = status.get("internet_learning", {}) if isinstance(status, dict) else {}
        state = info.get("state", "aus")
        mode = info.get("mode", "continuous_internet_learning")
        label_state = {"aktiv": "Aktiv", "bereit": "Aktiv", "pausiert": "Pausiert", "aus": "Aus"}.get(state, str(state))
        self.internet_learning_state.configure(text=f"Internet-Lernen: {label_state} | Modus: {mode}")
        self.internet_learning_source.configure(text=f"Letzte Quelle: {info.get('last_source') or '-'}")
        self.internet_learning_time.configure(text=f"Letzte Lernzeit: {info.get('last_learning_time') or '-'}")
        self.internet_learning_findings.configure(text=f"Neue Funde: {info.get('new_findings', 0)}")
        limit = info.get("bandwidth_limit_percent", 10)
        active = "ja" if info.get("bandwidth_limit_active", True) else "nein"
        self.internet_learning_limit.configure(text=f"Bandbreitenlimit aktiv: {active} | Limit: {limit} %")
        if info.get("enabled", False):
            self.internet_learning_button.configure(text="Internet-Lernen deaktivieren")
        else:
            self.internet_learning_button.configure(text="Internet-Lernen aktivieren")

    def toggle_internet_learning(self):
        learner = getattr(self.system, "internet_learning", None)
        if not learner:
            self.footer_label.configure(text="Internet-Lernen pausiert: Internet nicht verfügbar oder keine verwertbaren Quellen.")
            return
        status = learner.status()
        if status.get("enabled", False):
            if hasattr(learner, "set_enabled"):
    ...
    def copy_output(self):
        ...
        if not text.strip():
            self.footer_label.configure(text="Keine Ausgabe zum Kopieren.")
            self.after(3500, self._refresh_status)
            return
        self.clipboard_clear()
        self.clipboard_append(text)
        self.update_idletasks()
        self.footer_label.configure(text="Ausgabe in die Zwischenablage kopiert.")
        self.after(3500, self._refresh_status)
    ...
    def _enable_file_drop(self):
        try:
            ...
            if hasattr(self.input_box, "drop_target_register"):
                self.input_box.drop_target_register("DND_Files")
                self.input_box.dnd_bind("<<Drop>>", self._handle_file_drop)
            else:
                self.tk.call("tkdnd::drop_target", "register", self.input_box._w, "DND_Files")
                self.input_box.bind("<<Drop>>", self._handle_file_drop)
            self.file_drop_status = "Drag-and-Drop für Dateien im Eingabefeld aktiv."
        except Exception:
            self.file_drop_status = "Drag-and-Drop nicht verfügbar; Datei-Buttons bleiben aktiv."
    ...
    def _handle_file_drop(self, event):
        ...
        for path in paths:
            if path.is_dir():
                commands.append(f'lerne aus Ordner "{path}"')
            else:
                commands.append(f'lerne aus Datei "{path}"')
        self.input_box.insert("insert", ("\n" if self.input_box.get("1.0", "end").strip() else "") + "\n".join(commands))
        self.log_activity("FileAgent", f"{len(commands)} Datei-/Ordnerpfad(e) per Drag-and-Drop übernommen.")
        return "break"

    def open_file_prompt(self):
        path = filedialog.askopenfilename(parent=self, title="Datei öffnen")
        if path:
            self._insert_file_command("lies Datei", path)

    def learn_file_prompt(self):
        path = filedialog.askopenfilename(parent=self, title="Datei lernen")
        if path:
            self._insert_file_command("lerne aus Datei", path)

    def learn_folder_prompt(self):
        path = filedialog.askdirectory(parent=self, title="Ordner lernen")
        if path:
            self._insert_file_command("lerne aus Ordner", path)

    def _insert_file_command(self, command: str, path: str):
        self.input_box.delete("1.0", "end")
        self.input_box.insert("1.0", f'{command} "{path}"')
        self.log_activity("FileAgent", f"Auftrag vorbereitet: {command} {path}")
    ...
    def ask(self, event=None):
        ...
        self.log_activity("Suchmodus", self.search_mode.get())
        ...
        if hasattr(self.system, "set_search_mode"):
            self.system.set_search_mode(self.search_mode.get())
