...
def main() -> int:
    ...
    required = (
        ...
        "17_tests/test_foundation_reasoning_4_1_coverage.py",
        "17_tests/test_v34_0_version_consistency.py",
    ...
    manifest = json.loads((ROOT / "11_gui/GUI_34_0_MANIFEST.json").read_text(encoding="utf-8"))
    if manifest.get("version") != EXPECTED_VERSION:
        issues.append("GUI-Manifest ist nicht 34.0.")
    ...
    if database.is_file():
        with sqlite3.connect(f"file:{database}?mode=ro", uri=True) as connection:
            ...
            if not memory_table:
                ...
                guiding_rows = connection.execute(
                    """WITH latest AS (
                           ...
                           GROUP BY COALESCE(json_extract(metadata, '$.key'), content)
                       )
                       SELECT COUNT(*), COUNT(DISTINCT json_extract(memory.metadata, '$.rule_id'))
                       FROM foundation_memory AS memory
                       JOIN latest ON latest.id = memory.id
                       WHERE json_extract(memory.metadata, '$.guiding_principle') = 1
                         AND json_extract(memory.metadata, '$.policy_status') = 'active_provisional'"""
                ).fetchone()
            ...
            if not reasoning_table:
                issues.append("Foundation Reasoning Layer 4.1 fehlt.")
            else:
                ...
                source = (ROOT / "01_system/kontinuum/core/foundation_reasoning.py").read_text(encoding="utf-8")
                if 'VERSION = "4.1"' not in source:
                    issues.append("Foundation Reasoning Layer ist nicht auf 4.1 migriert.")
                uncovered = connection.execute(
                    """SELECT COUNT(*) FROM foundation_decisions AS decision
                       WHERE decision.kind = 'foundation.decision'
                         AND json_extract(decision.metadata, '$.reasoning_required') = 1
                         AND NOT EXISTS (
                           SELECT 1 FROM foundation_reasoning AS trace
                           WHERE trace.kind = 'foundation.reasoning.decision'
                             AND json_extract(trace.metadata, '$.decision_id') = decision.id
                         )"""
                ).fetchone()[0]
                if uncovered:
                    issues.append(f"Reasoning-Coverage unvollstaendig: {uncovered} Pflichtentscheidungen ohne Trace.")
